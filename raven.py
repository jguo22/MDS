...
class Raven:
    ...
    def __read_value(
        ...
        message = Raven.__make_message(
            message_type, Raven.__ReadWrite.READ, channel)
    ...
    def get_motor_mode(self, motor_channel: MotorChannel, retry=0):
        ...
        """
        assert isinstance(motor_channel, Raven.MotorChannel)
    ...
    def get_motor_pid(self, motor_channel: MotorChannel, retry=0):
        ...
        assert isinstance(motor_channel, Raven.MotorChannel)
        value = self.__read_value(
            Raven.__MessageType.MOTOR_PID, motor_channel, retry)
    ...
    def get_servo_position(
        ...
        """
        assert isinstance(servo_channel, Raven.ServoChannel)
    ...
    def get_odometry(self, retry=0):
        value = self.__read_value(
            Raven.__MessageType.ODOMETRY, retry=retry
        )
    ...
    def set_angle(self, angle: float, retry=0):
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            cal_vel = (new_encoder - current_encoder) / \
